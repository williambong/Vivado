...
extern "C" void duc ( int18 din_i,  uint16 freq,  int18* dout_i,  int18* dout_q);

#define duc AESL_ORIG_DUT_duc
extern "C" void duc ( int18 din_i,  uint16 freq,  int18* dout_i,  int18* dout_q);
#undef duc

void duc ( int18 din_i,  uint16 freq,  int18* dout_i,  int18* dout_q)
{
			...
			while (AESL_token != "[[/transaction]]")
			{
				...
				while (!no_x)
				{
					...
					if (x_found != string::npos)
					{
						if (!err)
						{
							cerr << "@W [SIM-201] RTL produces unknown value 'X' on port 'dout_i', possible cause: There are uninitialized variables in the C design." << endl;
							err = true;
						}
			...
			while (AESL_token != "[[/transaction]]")
			{
				...
				while (!no_x)
				{
					...
					if (x_found != string::npos)
					{
						if (!err)
						{
							cerr << "@W [SIM-201] RTL produces unknown value 'X' on port 'dout_q', possible cause: There are uninitialized variables in the C design." << endl;
							err = true;
						}
						AESL_token.replace(x_found, 1, "0");
					}
...
// [call_c_dut] ---------->
		...
		AESL_ORIG_DUT_duc(din_i, freq, dout_i, dout_q);
